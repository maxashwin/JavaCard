...
public class JCAlgTestApplet extends javacard.framework.Applet 
{
 ...
 /* Persistent : 68 bytes,  Transient(Deselect) : 0 bytes,  Transient(Reset) : 0 bytes */
        m_supportTest = new AlgSupportTest();

 /* Persistent : 2112 bytes,  Transient(Deselect) : 632 bytes,  Transient(Reset) : 632 bytes */
        m_perfTest = new AlgPerformanceTest();

 /* Persistent : 228 bytes,  Transient(Deselect) : 52 bytes,  Transient(Reset) : 52 bytes */
